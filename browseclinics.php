...
<html lang="en">
    ...
    <body id="page-top">
        ...
        <header class="masthead">
            ...
            <?php $sql = "SELECT distinct c.id as clinicid, c.clinicname as clinicname, c.mobile as mobile, c.address as address, c.area as area FROM clinic c, doctor d where (c.id, d.id) IN (select clinicid, doctorid FROM venue) and area='$area' and specialization='$specialization'";
            $result = $conn->query($sql);
				if ($result->num_rows > 0) {
						...
						<div class="bg-white p-2 shadow rounded">
							<div class="row">
								...
								<div class="col-sm-3 mt-sm-5 col-12">
									<a href="browsedoctor.php?clinicid=<?php echo $clinicid;?>&spec=<?php echo $specialization;?>" type="button" class="btn d-none d-sm-block btn-block btn-outline-success">BOOK NOW <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
									<a href="browsedoctor.php?clinicid=<?php echo $clinicid;?>&spec=<?php echo $specialization;?>" type="button" class="btn d-block d-sm-none btn-sm btn-block btn-outline-success">BOOK NOW <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
								</div>
							</div>
						</div>
					<?php 
					...
					<div class="alert alert-danger"> No Hospitals found with this catagory in this area, to search <a href="index.php#findshop" class="font-weight-bold text-info">click here</a></div> 
                <?php } ?>
			</div>
        </header>
