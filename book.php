...
if (!isset($_SESSION['email'])) {
   ...
   header('location: login.php');
    ...
    if ($result->num_rows > 0) {
		...
		$degree = $rows['degree'];
		$image = $rows['image'];
		$sql1 = "SELECT * FROM venue where clinicid = $clinicid and doctorid=$doctorid";
		...
		$sql2 = "SELECT * FROM clinic where id = $clinicid";
		$result1 = $conn->query($sql2);
		if ($result1->num_rows > 0) {
			$rows2 = $result1->fetch_assoc();
			$shopname = $rows2['clinicname'];
		}
	}
}
?>
		  ...
		  <div class="col-sm-7 col-12">
			...
			<div class="desktop d-sm-block d-none text-left mt-sm-0">
					<p><i class="fas fa-phone"></i> +91 <?php echo $mobileno;?></p>
					    ...
					    <p><b>Time:</b> Min. <?php echo $time;?> per patient</p>
			</div>
			<div class="desktop d-sm-none d-block text-left mt-2 small">
					<p><i class="fas fa-phone"></i> +91 <?php echo $mobileno;?></p>
							<p class="small"><b>Timings:</b> <?php if($mon){echo "Monday: $mon<br>";}?>
                             ...
                             <?php if($sat){echo "Saturday: $sat<br>";}?>
                             <?php if($sun){echo "Sunday: $sun";}?></p>
							 	<p class="small"><b>Hospitals:</b> <?php echo $shopname;?></p>
							 	<p class="small"><b>Time per Patient:</b> <?php echo $time;?></p>
			</div>
		   </div>
		 </div>
		 <hr>
		 <form method="POST" enctype="multipart/form-data" action="insertdetails.php"  class="was-validated">
			...
			<input type="text" value="<?php echo $clinicid;?>" style="display:none" name="clinicid"/>
            ...
            <div class="text-center">
				<button class="btn btn-primary btn-xl text-uppercase" name="generate" type="submit">Get Token</button>
            </div>	
         </form>
		</div>	
	  </div>
