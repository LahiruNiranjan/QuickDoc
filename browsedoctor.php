...
if (!isset($_SESSION['email'])) {
   ...
   $clinicid = test_input($_GET['clinicid']);
   $spec = test_input($_GET['spec']);
   $sql = "SELECT * FROM clinic where id ='$clinicid'";
...
<header class="masthead text-dark">
	<div class="container-fluid">
		...
		<hr>
		<?php $sql = "SELECT * FROM venue where clinicid = $clinicid";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
					...
					<div class="bg-white p-2 mt-2 shadow rounded">
						<div class="row">
							...
							<div class="col-sm-3 mt-sm-5 col-12">
								<a href="book.php?doc=<?php echo $doctorid;?>&shop=<?php echo $clinicid?>" type="button" class="btn d-none d-sm-block btn-block btn-success">GET TOKEN <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
								<a href="book.php?doc=<?php echo $doctorid;?>&shop=<?php echo $clinicid?>" type="button" class="btn d-block d-sm-none btn-sm btn-block btn-outline-success">GET TOKEN <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
							</div>
						</div>
					</div>
				<?php 
